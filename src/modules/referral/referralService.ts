        ...
        if (authUser.notifications?.promotional?.email !== false) {
          ...
          void SendEmail.sendRawEmail(
            template,
            {
              ...
              "{COACH_FIRSTNAME}": `${(authUser.fullname ?? "").split(" ")[0] || "Your Coach"}`,
              "{COACH_LASTNAME}": `${(authUser.fullname ?? "").split(" ").slice(1).join(" ") || ""}`,
              "{COACH_FIRSTNAME} {COACH_LASTNAME}": `${authUser.fullname ?? "Your Coach"}`,
            },
            [email],
            subject,
            null
